...
class GameModel:
    def __init__(self):
        self.__level = 1
        self.__bricks = self.__generateBricks(Settings.ROWS_LEVEL[self.__level - 1])
        self.__bar = Bar()
        bullet_start = {"x": self.__bar.xpos - (Settings.BALL_RADIUS / 2), "y": self.__bar.ypos - Settings.BALL_RADIUS}
        self.__bullet = Bullet(bullet_start)
        self.__score = 0
        self.__running = True


    def __generateBricks(self, qnt: int) -> List: # Generate and return a list of bricks with the right coordinates
        createdbricks = []
    ...
    def step(self) -> int:
        ...
        if self.__bullet.move():
            ...
            if rect_circ_collide(self.__bar.xpos, self.__bar.ypos, self.__bar.width, self.__bar.height, self.__bullet.pos[0], self.__bullet.pos[1], self.__bullet.radius):
                self.__bullet.hit(self.__bar.xpos, self.__bar.ypos, True)
            for brick in self.__bricks: # Check if any of the bricks is hit by the bullet
                if brick.active and rect_circ_collide(brick.xpos, brick.ypos, brick.width, brick.height, self.__bullet.pos[0], self.__bullet.pos[1], self.__bullet.radius):
                    self.__bullet.hit(brick.xpos, brick.ypos, False)
                    self.__score += 1
                    brick.explode()
                    if not self.__bricks: # If there are bricks left
