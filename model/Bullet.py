...
class Bullet:
    def __init__(self, barpos: Dict):
        self.__pos = {"x": barpos["x"], "y": barpos["y"] - Settings.DEFAULT_BAR_HEIGHT - 5}
    ...
    def move(self) -> bool: # Return None if the bullet touches the ground, return current position otherwise
        ...
        if (self.__pos["x"] + self.__radius) + self.__velocity["x"] >= Settings.WIDTH or (self.__pos["x"] - self.__radius) + self.__velocity["x"] <= 0:
